...
class Categories extends CI_Controller {
    ...
    public function index() {
     	...
     	$data['categories1'] = $this->categories_model->get_parent2();
	...
	$k = 0;
		...
		if($this->categories_model->get_products1($wow,'','','','')){
			$totalRec = count($this->categories_model->get_products1($wow,'','','',''));
		}
		else{
			$totalRec = 0;
		}
		...
		$config["base_url"] = base_url() . "admin/categories/searchkey?searchkey=".$wow."&sort_by=".$sort_by."&sort_order=".$sort_order;
	...
	$categories1= $this->categories_model->get_products1($wow, $config["per_page"], $per_page,$sort_by,$sort_order);
    	$table = '';	
    	$pstatus= '';	
    	$table .= '<table class="table table-bordered table-sortable">
                      <thead>
                        <tr>
                          <th>ID</th>
                          <th href="'.base_url() .'admin/categories/searchkey?searchkey='.$wow.'&sort_by=title&sort_order='.(($sort_order == 'asc') ? 'desc' : 'asc').'" class="sortable moiz sort-alpha sort-'.(($sort_order == 'desc') ? 'desc' : 'asc').'">Category</th>
                          <th href="'.base_url() .'admin/categories/searchkey?searchkey='.$wow.'&sort_by=description&sort_order='.(($sort_order == 'asc') ? 'desc' : 'asc').'" class="sortable moiz sort-alpha sort-'.(($sort_order == 'desc') ? 'desc' : 'asc').'">Descrption</th>
				...
				$k = $config["per_page"];
                      ...
                      $table .= '  <tr>
                          ...
                          <td style="text-align:center;"><a href="'. base_url().'admin/Categories/edit_categories/'. $value['id'].'" class="fa fa-pencil color-red"></a></td>
                        </tr> ';
                         } } else { 
                        $table .= '<tr>
    ...
    public function update_status(){
       ...
       $this->categories_model->status_update($id,$data);
