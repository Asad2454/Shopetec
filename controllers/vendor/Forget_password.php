...
class Forget_password extends CI_Controller {
    ...
    public function reset() {
        ...
        $id = strstr($id, '@');
        $data['id'] = ltrim($id, '@');
        $this->load->view('vendor/new_password',$data);
    }
    ...
    public function change_password() {
        ...
        if($password == $password1){
            ...
            redirect('vendor/forget_password/reset/?code='.md5($password).'@'.$id);
        }
    }
    
    public function check_email() {
        $email1 = $this->input->post('email');
        $sql = "SELECT * FROM vendors WHERE email = ? AND status != '0' ";
        ...
        if ($val->num_rows()) {
            ...
            $message = "Hi " .$name.", Click the link below to reset your password. " . base_url()."vendor/forget_password/reset/?code=".$pass."@".$vid." If you did not request this password change, please ignore this email - your password will remain unaffected ";
            ...
            redirect('vendor/forget_password');
        }
        else{
