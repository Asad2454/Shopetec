...
class Products_model extends CI_Model {
    ...
    public function get_specific_products($ids){
    ...
    public function get_products1($key, $limit, $start,$sort_by,$sort_order,$ids){
    	$id = $_SESSION['mcategory'];
    	array_push($ids,$id);
    	$this->db->select('*,vendors.name as vname,vendors.id as vid,products.id as pid,products.status as pstatus');
    	$this->db->from('products');
    	$this->db->join('vendors', 'vendors.id = products.user_id','left');
    	if(!empty($key)){
