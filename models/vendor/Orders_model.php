...
class Orders_model extends CI_Model {
    ...
    public function update_orders_status($id,$data) {
		...
		$this->db->where_in('products.user_id', $_SESSION['vid']);
		$query = $this->db->get();
    ...
    public function neworders($key, $limit, $start, $sort_by, $sort_order) {
		$this->db->select('orders.id as oid, products.id as pid,order_id,date,orders.status as ostatus,customer.name as fname,customer.last_name as lname');
		$this->db->from('orders');
		$this->db->join('customer','customer.id = orders.user_id');
		$this->db->join('attribute_product','attribute_product.id = orders.product_id');
		$this->db->join('products','products.id = attribute_product.product_id');
		if(!empty($key)){
        	...
        	$this->db->where('products.user_id', $_SESSION['vid']);
    		$this->db->where('orders.status', 'new');
		}
	    else{
        	$this->db->where('products.user_id', $_SESSION['vid']);
    		$this->db->where('orders.status', 'new');
    ...
    public function processorders($key, $limit, $start, $sort_by, $sort_order) {
	  $this->db->select('orders.id as oid, products.id as pid,order_id,date,orders.status as ostatus,customer.name as fname,customer.last_name as lname');
		$this->db->from('orders');
		$this->db->join('customer','customer.id = orders.user_id');
		$this->db->join('attribute_product','attribute_product.id = orders.product_id');
    ...
    public function allorders($key, $limit, $start, $sort_by, $sort_order) {
	   $this->db->select('orders.id as oid, products.id as pid,order_id,date,orders.status as ostatus,customer.name as fname,customer.last_name as lname');
		$this->db->from('orders');
		$this->db->join('customer','customer.id = orders.user_id');
		$this->db->join('attribute_product','attribute_product.id = orders.product_id');
    ...
    public function processcomplete($key, $limit, $start, $sort_by, $sort_order) {
	    $this->db->select('orders.id as oid, products.id as pid,order_id,date,orders.status as ostatus,customer.name as fname,customer.last_name as lname');
		$this->db->from('orders');
		$this->db->join('customer','customer.id = orders.user_id');
		$this->db->join('attribute_product','attribute_product.id = orders.product_id');
